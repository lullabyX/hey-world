...
export default function NoiseViewer() {
  ...
  useEffect(() => {
    ...
    const sampleContinental = (gx: number, gz: number) => {
      // Two-stage domain warp to better mimic Minecraft's macro continents
      const wx1 =
      ...
      const warp2Scale = scale * 0.9;
      const wx2 =
        noises.warpNoise.noise(
          (gx - 500) / warp2Scale,
          (gz + 500) / warp2Scale
        ) *
        (warpStrengthClamped * 0.35);
      const wz2 =
        noises.warpNoise.noise(
          (gx + 1500) / warp2Scale,
          (gz - 1500) / warp2Scale
    ...
    for (let iy = 0; iy < HEIGHT; iy++) {
      for (let ix = 0; ix < WIDTH; ix++) {
        ...
        const continental0 = sampleContinental(gx, gz);
        const continentalness = continental0;
